    ...
    try{
        ...
        if (!subject || !description || !priority) {
            logEvent('Validation Error: Missing ticket fields', 'ticket', 
                {subject, description, priority}, 'warning'
            )
        ...
        logEvent(`Ticket created successfully: $(ticket.id)`, 'ticket',
            { ticketId: ticket.id }, 'info'
        );
        ...
        logEvent('An error occured while creating the ticket', 'ticket', 
            { formData: Object.fromEntries(formData.entries())}, 'error', error
        )
...
export async function getTickets() {
    try{
        ...
        if (!user){
            logEvent('Unauthorized access t ticket list', 'ticket', {}, 'warning');
            return [];
        }
...
export async function getTicketById(id: string) {
    try{
        ...
        logEvent('Error fetching ticket details', 
...
export async function closeTicket(prevState: {success: boolean; message: string}, formData: FormData): Promise<{success: boolean, message: string}> {
    ...
    if (!ticket || ticket.userId !== user.id) {
        logEvent('Unaithorized ticket close attempt', 'ticket', { ticketId, userId: user.id }, 'warning');
